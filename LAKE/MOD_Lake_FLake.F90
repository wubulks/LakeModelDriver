...
! -------------------------------------------------------------------------------------
   USE MOD_Lake_Const, only: stefnc, omega, c_lwrad_emis, h_Ice_min_flk, h_Snow_min_flk,&
                              tpl_rho_I, tpl_rho_w_r, d2r, tfrz, tpsf_nu_u_a
...
!------------------------------------------------------------------------------
      ...
      IF (DEF_External_Lake%DEF_USE_SURFACE_LAYER) THEN
            !-WMEJ After testing, we found that the u_star_w_flk calculated by this method is not satisfactory.
            !      Should be checked again. This is just a temporary solution now.
            mmtum = -ustar**2 
            mmtum = MIN( -tpsf_nu_u_a * U_a / hwref, mmtum, 0.0) * arhos
            u_star_w_flk = SQRT(-mmtum/tpl_rho_w_r)   
      ...
      ELSE
         ...
         ELSE
            ...
            CALL FLake_SurfFlux ( &
                  ...
                  ! "out" arguments
                  ! ---------------------------
                  mmtum       , ustar       , fsena         , lfevpa       , fevpa       ,&
                  olrg        , fgrnd       )
...
!------------------------------------------------------------------------------
      ...
      IF (icedp < h_Ice_min_flk) THEN 
         DO i = 1, nlake
            icefr(i) = 0.
            wice_lake(i) = 0.
            wliq_lake(i) = tpl_rho_w_r*dzlake(i)
         ENDDO        
      ELSE
         DO i = 1, nlake
            IF (icedp > zilak(i+1)) THEN
               icefr(i) = 1.
               wice_lake(i) = tpl_rho_I*dzlake(i)
               wliq_lake(i) = tpl_rho_w_r*dzlake(i)
            ELSE 
...
!================================================================================
   ...
   SUBROUTINE FLake_SurfFlux ( &
               ...
               ! "out" arguments
               ! ---------------------------
               Q_momentum   , u_star_st    , Q_sensible   , Q_latent    , Q_watvap   ,&
               Q_olrg       , Q_fgrnd      )
