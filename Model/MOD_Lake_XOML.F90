...
!  ------------------------- inout variables ---------------------------
   ...
   real(r8), intent(inout)  :: &
      dzlake(nlayer)           ,&! lake layer thickness (m)
...
!================================================================================
      ...
      !------------------------------------------------------------------------------
      IF (DEF_External_Lake%DEF_USE_COLML_FLUX_SCHEME) THEN
         lb = snlay + 1
         ...
         CALL CoLML_SurfFlux ( &
               ...
               hpbl = hpbl            , dzlaketop = dzlake(1) , zlakebot = zlake(nlayer), dzssbtop = dzssb(lb)  ,&
      ...
      ENDIF
         ...
         CALL XOML_solve( &
               ...
               ! "inout" arguments
               ! -------------------
               dzlake = dzlake          , zlake = zlake       , zilak = zilak           , lktmp = lktmp         ,&
...
!  ------------------------- local variables ---------------------------
   ...
   real(r8) :: gdbuo(nlayer)    ! gradient buoyancy [s-2] dB/dzlake
...
!  ------------------------- input variables ---------------------------
   ...
   real(r8), intent(in)     :: &
      dzlake(nlayer)           ,&! layer thickness (m)
...
!================================================================================
      ...
      IF (tskBC) THEN  ! USE the air-water interface temperature as the boundary condition
         ...
         IF (l1.eq.3)  THEN
            k = 1
            cp  = lkrho(k) * cpliq     
            dtz = 2 * dtlak / dzlake(k)
            pdz = dzlake(k)
            ndz = dzlake(k) + dzlake(k+1)

            B(k) = 0.0
            D(k) = 1.0 + (1-beta) * eta * dtz *  &
                  ( nhf(nlayer, k, dzlake, Kh ) / ndz )
            A(k) = (1-beta) * eta * dtz * nhf(nlayer, k, dzlake, Kh ) / ndz
            X(k) = lktmp(k)                                                                        &!- current temperature
               + beta * dtz * nhf(nlayer, k, dzlake, Kh )/ndz * (lktmp(k+1)-lktmp(k))             &!- Heat flux from the layer below 
               + dtlak / dzlake(k) * Kh(1) * 2 * (tskin-lktmp(1)) / dzlake(1)                      &!- Heat flux from the air-water interface
               + dtlak / dzlake(k) * (1 - btpri) * sabg / cp * (prfsl(k) - prfsl(k+1))            &!- Solar radiation heating term
         ...
         X(k) = lktmp(k)                                                                              &!- current temperature
                  + beta * dtz * nhf(nlayer, k, dzlake, Kh )/ndz * (lktmp(k+1) - lktmp(k))               &!- Heat flux from the layer below
                  + dtlak / dzlake(k) * fgrnd1 / cp                                                      &!- Heat flux from the air-water interface
                  + dtlak / dzlake(k) * (1 - btpri) * sabg / cp * (prfsl(k) - prfsl(k+1))                &!- Solar radiation heating term
                  + dtlak * rlx                                                                          !- Relaxation term (correction for SST Nudging)
      ...
      DO k = 2, nlayer-1
         ...
         cp  = lkrho(k) * cpliq
         dtz = 2 * dtlak / dzlake(k)
         pdz = dzlake(k) + dzlake(k-1)
         ndz = dzlake(k) + dzlake(k+1)

         B(k) = (1 - beta)  *  dtz  *  phf(nlayer, k, dzlake, Kh ) / pdz
         D(k) = 1.0 + (1 - beta)  *  dtz  *   &
                  ( nhf(nlayer, k, dzlake, Kh ) / ndz + phf(nlayer, k, dzlake, Kh ) / pdz )
         A(k) = (1 - beta)  *  dtz  *  nhf(nlayer, k, dzlake, Kh ) / ndz
         X(k) = lktmp(k)                                                                             &!- current temperature
                  + beta * dtz * ( nhf(nlayer, k, dzlake, Kh ) / ndz * (lktmp(k+1) - lktmp(k))          &!- Heat flux from the layer below
                  - phf(nlayer, k, dzlake, Kh ) / pdz * (lktmp(k) - lktmp(k-1)) )                       &!- Heat flux from the layer above
                  + dtlak / dzlake(k) * (1 - btpri) * sabg / cp * (prfsl(k) - prfsl(k+1))                !- Solar radiation heating term
...
!================================================================================
      ...
      DO k = 2, nlayer-1
         pdz = (dzlake(k) + dzlake(k-1)) / 2.
         ndz = (dzlake(k) + dzlake(k+1)) / 2.
         n2(k) = grav / lkrho(k) * (nhf(nlayer, k, dzlake, lkrho) - phf(nlayer, k, dzlake, lkrho)) / dzlake(k)
         s2(k) = ((nhf(nlayer, k, dzlake, uwatv) - phf(nlayer, k, dzlake, uwatv)) / dzlake(k))**2 + &
                  ((nhf(nlayer, k, dzlake, vwatv) - phf(nlayer, k, dzlake, vwatv)) / dzlake(k))**2
      ENDDO
...
!================================================================================
      ...
      !- USE in the tridiagonal matrix solver 
      ! l1 = 2; lm = 2; w1 = - tau / lkrho(1) * (prfmt(1) - prfmt(2)) * dzlake(1) / Km (1); wm = 0_r8     ! Noh 2011
      l1 = 2; lm = 1; w1 = - tau / lkrho(1) * (prfmt(1) - prfmt(2)) * dzlake(1) / Km (1); wm = 0_r8       ! Lei Sun or Tiejun Ling modified
      ...
      DO k = 2, nlayer-1
         ...
         dtz = 2_r8 * dtlak / dzlake(k)
         pdz = dzlake(k) + dzlake(k-1)
         ndz = dzlake(k) + dzlake(k+1)

         B(k) = (1_r8 - beta) * dtz * phf(nlayer, k, dzlake, Km ) / pdz
         D(k) = 1_r8 + (1_r8 - beta) * dtz *  &
                  ( nhf(nlayer, k, dzlake, Km ) / ndz + phf(nlayer, k, dzlake, Km ) / pdz )
         A(k) = (1_r8 - beta) * dtz * nhf(nlayer, k, dzlake, Km ) / ndz
         X(k) = watv(k)                                                                         &!- current velocity
                  + beta * dtz * ( nhf(nlayer, k, dzlake, Km ) / ndz * (watv(k+1) - watv(k))       &!- production by mean velocity shear
                                 - phf(nlayer, k, dzlake, Km ) / pdz * (watv(k) - watv(k-1)))      &
                  + dtlak / dzlake(k) * tau/lkrho(k) * (prfmt(k) - prfmt(k+1))                   !&!- Wind stress term
                  ! + dtlak * fcor * watv(k)
      ENDDO
...
!================================================================================
      ...
      IF (l1 .eq. 3) THEN
         ...
         B(k) = 0.0
         D(k) = 1.0 + (1 - beta) *  dtz *  &
                  ( nhf(nlayer, k, dzlake, Ke) / ndz )
         A(k) = (1 - beta) *  dtz * nhf(nlayer, k, dzlake, Ke) / ndz
         X(k) = tke(k)                                                                         &!- current TKE
                  + beta * dtz* nhf(nlayer, k, dzlake, Ke) / ndz * (tke(k+1) - tke(k))            &!- TKE diffusion term
                  + dtlak / dzlake(k) * w1                                                         !- Top boundary condition
      ...
      DO k = 2, nlayer-1
         ...
         B(k) = (1 - beta) * dtz * phf(nlayer, k, dzlake, Ke) / pdz
         D(k) = 1.0 + (1 - beta) * dtz *  &
                  ( nhf(nlayer, k, dzlake, Ke) / ndz + phf(nlayer, k, dzlake, Ke) / pdz )   &
                  + eps(k) * dtlak / (0.5 * qcl(k) * qcl(k))
         A(k) = (1 - beta) * dtz * nhf(nlayer, k, dzlake, Ke) / ndz
         X(k) = tke(k)                                                                                              &!- current TKE
                  ! + dtlak*Kh (k)*(nhf(nlayer,k,dzlake,buo)-phf(nlayer,k,dzlake,buo))/dzlake(k)  &
                  - dtlak * Kh (k) * n2(k)                                                                            &!- TKE production by bouyancy flux
                  + beta * dtz * ( nhf(nlayer, k, dzlake, Ke) / ndz * (tke(k+1) - tke(k))                              &!- TKE diffusion term
                                 - phf(nlayer, k, dzlake, Ke) / pdz * (tke(k) - tke(k-1)) )                            &
                  + dtlak * Km (k) * ((nhf(nlayer, k, dzlake, uwatv) - phf(nlayer, k, dzlake, uwatv)) / dzlake(k))**2    &!- TKE production by mean velocity shear
                  + dtlak * Km (k) * ((nhf(nlayer, k, dzlake, vwatv) - phf(nlayer, k, dzlake, vwatv)) / dzlake(k))**2
...
!================================================================================
      ...
      DO k = 2, nlayer-1
         ...
         p_tke(k) = Km (k) * ((nhf(nlayer, k, dzlake, uwatv) - phf(nlayer, k, dzlake, uwatv)) / dzlake(k))**2 + &
                  Km (k) * ((nhf(nlayer, k, dzlake, vwatv) - phf(nlayer, k, dzlake, vwatv)) / dzlake(k))**2
         d_tke(k) = (nhf(nlayer, k, dzlake, Ke) / ndz * (tke(k+1) - tke(k))        &
                  - phf(nlayer, k, dzlake, Ke) / pdz * (tke(k) - tke(k-1))) / dzlake(k)
         n2(k) = grav / lkrho(k) * (nhf(nlayer, k, dzlake, lkrho) - phf(nlayer, k, dzlake, lkrho)) / dzlake(k)
         b_tke(k) = -Kh (k) * n2(k)
         ! b_tke(k) = Kh (k) * (nhf(nlayer, k, dzlake, buo) - phf(nlayer, k, dzlake, buo)) / dzlake(k)
         s2(k) = ((nhf(nlayer, k, dzlake, uwatv) - phf(nlayer, k, dzlake, uwatv)) / dzlake(k))**2    &
               + ((nhf(nlayer, k, dzlake, vwatv) - phf(nlayer, k, dzlake, vwatv)) / dzlake(k))**2
...
!================================================================================
      ...
      DO k = 2, nlayer-1
         gdbuo(k) = (buo(k+1) - buo(k-1)) / (dzlake(k+1) * 0.5 + dzlake(k) + 0.5 * dzlake(k-1))
      ENDDO
...
!================================================================================
      SELECT CASE(l1)
         CASE(1)     ! p(1) = w1
            e(1) = 0.0
            f(1) = w1
         CASE(2)     ! p(1) = p(2)- w1  xxx -k(p(2)-p(1))/dzlake=wx
            e(1) = 1.0
