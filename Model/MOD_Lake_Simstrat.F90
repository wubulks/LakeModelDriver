...
MODULE  strat_forcing
   ...
   PUBLIC :: Simstrat_SurfFlux
   PUBLIC :: Simstrat_FluxAdd
...
!-----------------------------------------------------------
   ...
   CONTAINS
      ...
      procedure, pass :: FluxUpdate => FluxUpdate
      procedure, pass :: update => SurfFlux
...
!-----------------------------------------------------------
   ...
   subroutine FluxUpdate ( self, state,&
                           ! "in" arguments
                           ! ---------------------------
                           usurf    , vsurf       , filtw    , tmref   ,&
                           tskin    , precip      , rad0     , heat    ,&
                           stateC10 , u_taus      , taux     , tauy    ,&
                           olrg     , fsena       , lfevpa   , lwdns   ,&
                           heat_ice , heat_snow   , heat_snowice)
   ...
   ! ----- input variables -----
   real(RK), intent(in)     :: &
      usurf                   ,&! Wind speed [m/s]
      vsurf                   ,&! Wind speed [m/s]
      filtw                   ,&! filtered wind [m/s]
      tmref                   ,&! Air temperature [K]
      tskin                   ,&! Lake surface temperature [K]
      precip                  ,&! Precipitation [mm/s]
      rad0                    ,&! The remaining solar radiation after being absorbed by the surface [W/m²]
      lwdns                   ,&! Downward longwave radiation at surface [W/m²]
      heat                    ,&! Heat flux [W/m²]
      heat_snow               ,&! Heat snow
      heat_snowice            ,&! Heat snowice
      heat_ice                ,&! Heat ice
      stateC10                ,&! Drag coefficient
      u_taus                  ,&! Friction velocity
      taux                    ,&! wind stress: E-W [kg/m/s**2]
      tauy                    ,&! wind stress: N-S [kg/m/s**2]
      olrg                    ,&! Outgoing longwave radiation [W/m²]
      fsena                   ,&! Sensible heat flux [W/m²]
      lfevpa                    ! Latent heat flux [W/m²]
   !==============================================================================
      state%u10 = usurf * self%param%f_wind ! MS 2014: added f_wind
      state%v10 = vsurf * self%param%f_wind ! MS 2014: added f_wind
      state%Wf = filtw  ! AG 2014: added filtered wind
      state%uv10 = sqrt(state%u10**2 + state%v10**2) ! AG 2014
      state%SST = tskin - 273.15_RK  ! Lake surface temperature
      state%rad0 = rad0              ! The remaining solar radiation after being absorbed by the surface
      state%heat = heat               ! Heat flux
      state%T_atm = tmref - 273.15_RK  ! Air temperature
      state%precip = max(precip, 0.0_RK) ! Precipitation
      state%heat_snow = heat_snow
      state%heat_snowice = heat_snowice
      state%heat_ice = heat_ice
      state%C10 = stateC10
      state%u_taus = u_taus
      state%tx = taux
      state%ty = tauy
   ...
   end subroutine FluxUpdate



   subroutine Simstrat_SurfFlux( &
                                ! "in" arguments
                                ! ---------------------------
                                ...
                                ! "out" arguments
                                ! ---------------------------
                                heat_snow, heat_snowice, heat_ice, stateC10,&
                                u_taus   , taux        , tauy    , htvp    ,&
                                olrg     , fsena       , fseng   , lfevpa  ,&
                                fevpa    , fevpg       , F_glob  , fgrnd   ,&
                                fgrnd1   , rad0        , trad    , emsi     )
   ...
   ! Revisions:
   !     AG 2014
   !     Omarjan Obulkasim (WMEJ), 03/2024: Separated this subroutine in order for it to be called by other lake models
   !------------------------------------------------------------------
   use MOD_Lake_Const, only : tfrz, hvap, hsub, freez_temp, UseFilteredWind,&
                              IceModel, SnowModel, ForcingMode, f_wind, beta_sol,&
                              p_albedo, p_sw, p_lw, emiss_water, stefnc, cp_air,&
                              Hk_CH, Ha_a, Ha_b, Ha_c, l_h, l_e, lambda_snow, B0, c10,&
                              lambda_snowice, lambda_ice, emiss_ice, rho_air, hvap, hsub
   !==============================================================================
   ! ----- input variables -----
   real(RK)                           , intent(in)     :: &
      snwdp                                             ,&! Snow depth [m]
      wicedp                                            ,&! Ice water equivalent [m]
      bicedp                                            ,&! Ice thickness [m]
      usurf                                             ,&! Wind speed [m/s]
      vsurf                                             ,&! Wind speed [m/s]
      tmref                                             ,&! Air temperature [K]
      qmref                                             ,&! Specific humidity [g/kg]
      psurf                                             ,&! Air pressure [mbar]
      sabg                                              ,&! Solar radiation absorbed by ground [W/m²]
   ...
   real(RK)                           , intent(out)    :: &
      ...
      taux                                              ,&! wind stress: E-W [kg/m/s**2]
      tauy                                              ,&! wind stress: N-S [kg/m/s**2]
      htvp                                              ,&! Latent heat of vaporization or sublimation
      olrg                                              ,&! Longwave radiation
      fsena                                             ,&! Sensible heat flux
      fseng                                             ,&! Sensible heat flux
      lfevpa                                            ,&! Latent heat flux
      fevpa                                             ,&! Evaporation rate
      fevpg                                             ,&! Evaporation rate
      fgrnd                                             ,&! solar radiation absorbed by ground
      fgrnd1                                            ,&! solar radiation absorbed by ground
      rad0                                              ,&! The remaining solar radiation after being absorbed by the surface
      emsi                                              ,&! emissivity of the surface
      trad                                                ! Net radiation
      
   ! ----- local variables -----
   real(RK)                                            :: &
      T_surf                                            ,&! Lake surface temperature [°C]
      A_cur(8)                                          ,&! Current forcing data
      u10                                               ,&! Wind speed [m/s]
      v10                                               ,&! Wind speed [m/s]
      uv10                                              ,&! Wind speed [m/s]
      tau                                               ,&! Wind stress
      SST                                               ,&! Lake surface temperature [°C]
      Clouds                                            ,&! Cloudiness [0-1]
      heat                                              ,&! Heat flux [W/m²]
      T_atm                                             ,&! Air temperature [°C]
      Vap_atm                                           ,&! Vapour pressure [mbar]
      Vap_wat                                           ,&!  Water vapor saturation pressure in air at water temperature [milibar]
      Wf                                                ,&! Filtered wind speed [m/s]
      tx                                                ,&! Wind stress component in x-direction
      ty                                                ,&! Wind stress component in y-direction
      ...
      sh_c                                              ,&! Converter from absolut vapour pressure to specific humidity (Lepparanta 2015)
      qa                                                ,&! Specific humidity air
      q0                                                  ! Specific humidity for saturation levels (6.11 mbar) at surface (ice) at 0°C (Lepparanta 2015)
   !==============================================================================

      if((snwdp + wicedp + bicedp) > 0) then 
         T_surf = freez_temp                ! [°C] Ice cover
      else
         T_surf = tskin - tfrz              ! [°C] Free water
      end if

      ! ! Number of values to read, depending on filtered wind and precipitation
      ! if (UseFilteredWind .and. IceModel == 0) then
      !     nval_offset = 1
      ! else if (IceModel == 1 .and. UseFilteredWind) then
      !     nval_offset = 2
      ! else if (IceModel == 1) then
      !     nval_offset = 1
      ! else
      !     nval_offset = 0
      ! end if

      if (ForcingMode == 3 .and. IceModel == 4) then
      ...
      if (ForcingMode == 1) then
         ...
         ! call self%read (datum, A_s, A_e, A_cur, 4 + nval_offset, first_timestep) !-WMEJ No longer needed
         ! call self%read (datum, A_s, A_e, A_cur, 4 + nval_offset, first_timestep) !-WMEJ No longer needed
         !===========================================================
         ...
         SST = A_cur(3) ! Lake surface temperature
         
         ! rad0 = max(A_cur(4)*(1 - albedo_water)*(1 - beta_sol),0.0_RK) ! MS: added beta_sol and albedo_water
         rad0 = max(A_cur(4)*(1 - beta_sol),0.0_RK) ! MS: added beta_sol and albedo_water  !-WMEJ: The reflection of solar radiation must be completed in the lake driver
         heat = 0.0_RK
         T_atm = tmref - tfrz
         precip = 0.0_RK
         if (UseFilteredWind) then    !-WMEJ can not be used now
               Wf = A_cur(5) ! AG 2014
         end if


      !*** Forcing mode 2 (date, U10, V10, T_atm, H_sol, Vap)
      else if (ForcingMode >= 2) then
         if (ForcingMode == 2) then
               !===========================================================
               !-WMEJ Modified the way to read data
               A_cur(1) = usurf ! U10
               A_cur(2) = vsurf ! U10
               ...
               !-----------------------------------------------------------
               !-WMEJ Original way to read data
               ! call self%read (datum, A_s, A_e, A_cur, 5 + nval_offset, first_timestep)   !-WMEJ No longer needed
               !===========================================================
               ...
               if (bicedp > 0 .and. wicedp == 0 .and. snwdp == 0) then !-- Ice
                  ! F_glob = A_cur(4)*(1 - ice_albedo) * p_albedo               !-WMEJ The reflection of solar radiation must be completed in the lake driver
                  F_glob = A_cur(4) * p_albedo
               else if (wicedp > 0 .and. snwdp == 0) then !-- Snowice
                  ! F_glob = A_cur(4)*(1 - snowice_albedo) * p_albedo           !-WMEJ The reflection of solar radiation must be completed in the lake driver
                  F_glob = A_cur(4) * p_albedo
               else if (snwdp > 0) then !-- Snow
                  ! F_glob = A_cur(4)*(1 - snow_albedo) * p_albedo              !-WMEJ The reflection of solar radiation must be completed in the lake driver
                  F_glob = A_cur(4)* p_albedo
               else !-- Water
                  ! F_glob = A_cur(4)*(1 - albedo_water) * p_sw           !-WMEJ The reflection of solar radiation must be completed in the lake driver
                  F_glob = A_cur(4) * p_sw
               end if

               Vap_atm = A_cur(5)
               Cloud = 0.5
               if (UseFilteredWind) Wf = A_cur(6) ! AG 2014
               if (SnowModel == 1 .and. UseFilteredWind) then
                  precip = max(A_cur(7),0.0_RK)
               else if (SnowModel == 1) then
                  precip = max(A_cur(6),0.0_RK)
               end if


         !*** Forcing mode 3 (date, U10, V10, T_atm, H_sol, Vap, Clouds)
         else if (ForcingMode == 3) then
               !===========================================================
               !-WMEJ Modified the way to read data
               A_cur(1) = usurf ! U10
               A_cur(2) = vsurf ! U10
               ...
               !-----------------------------------------------------------
               !-WMEJ Original way to read data
               ! call self%read (datum, A_s, A_e, A_cur, 6 + nval_offset, first_timestep)  !-WMEJ No longer needed
               !===========================================================
               ...
               if (UseFilteredWind) Wf = A_cur(7) !AG 2014
               if (SnowModel == 1 .and. UseFilteredWind) then
                  precip = max(A_cur(8),0.0_RK)
               else if (SnowModel == 1) then
                  precip = max(A_cur(7),0.0_RK)
               end if
         ...
         !*** Forcing mode 4 (date, U10, V10, H_net, H_sol)
               ...
               A_cur(1) = usurf ! U10
               A_cur(2) = vsurf ! U10
               A_cur(3) = heat ! heat flux from forcing ??
               A_cur(4) = sabg  ! Solar radiation absorbed by ground
               !-----------------------------------------------------------
               !-WMEJ Original way to read data
               ! call self%read (datum, A_s, A_e, A_cur, 4 + nval_offset, first_timestep)  !-WMEJ No longer needed
               ...
               u10 = A_cur(1)*f_wind ! MS 2014: added f_wind
               v10 = A_cur(2)*f_wind ! MS 2014: added f_wind
               heat = A_cur(3) ! MS 2014
               ! F_glob = max(A_cur(4)*(1 - albedo_water) * p_sw,0.0_RK)      !-WMEJ The reflection of solar radiation must be completed in the lake driver
               F_glob = max(A_cur(4) * p_sw,0.0_RK) 
               T_atm = 0.0_RK
               if (UseFilteredWind) Wf = A_cur(5) ! AG 2014


         !*** Forcing 5 (date, U10, V10, T_atm, H_sol, Vap, ILWR)
         else if (ForcingMode == 5) then
               !===========================================================
               !-WMEJ Modified the way to read data
               A_cur(1) = usurf ! U10
               A_cur(2) = vsurf ! U10
         ...
         p_air = psurf/100.0_RK !-WMEJ: p_air = psurf, add in Lake driver
         if (ForcingMode /= 4) then ! Heat fluxes calculations (forcing 2, 3 and 5)
               if (bicedp + wicedp == 0) then !Free water
                  ...
                  fu = sqrt((2.7_RK*max(0.0_RK,(T_surf-T_atm)/(1 - 0.378_RK*Vap_atm/p_air))**0.333_RK)**2 + (0.6072_RK*3.1_RK*uv10)**2)
                  ...
                  Vap_wat = Vap_wat*(1 + 1e-6_RK*p_air*(4.5_RK + 0.00006_RK*T_surf**2))     !-WMEJ : p_air = psurf, add in Lake driver

                  ! Long-wave radiation according to Dilley and O'Brien
                  ! see Flerchinger et al. (2009)
                  if (ForcingMode /= 5) then
                  ...
                  H_A = H_A*p_lw ! Provided fitting factor p_radin (~1)
                  ...
                  heat_snow = 0 ! Heat snow
                  heat_snowice = 0 ! Heat snowice
                  heat_ice = 0 ! Heat ice

               else if (bicedp > 0 .or. wicedp > 0) then ! Ice Cover
                  ! Light penetration in snow, ice and snowice as well as wind blocking added 2018 by Love Raaman

                  if (T_atm >= Freez_Temp) then! Melting occures when air temp above freezing point,
                     ...
                     if (snwdp == 0) then ! Ice Cover (ice and snowice)
                           emissivity = emiss_ice
                     else ! Snow Cover
                           ! Varies from 0.8 to 0.9 depending on snow density
                           emissivity = 5.0e-4_RK * rhosnw + 6.75e-1_RK
                     end if
                     ! obs fitting factors p_lw and p_windf not applied to ice covered lake
                     if (ForcingMode /= 5) then
                           H_A = (Ha_a + Ha_b * (Vap_atm**(1.0_RK/2.0_RK))) * (1 + Ha_c * Cloud**2) * stefnc * (T_atm + 273.15_RK)**4
                     end if
                     H_W = -emissivity * stefnc * (T_surf + 273.15_RK)**4
                     H_K = rho_air * cp_air * Hk_CH * (T_atm - T_surf) * uv10
                     sh_c = 0.622/p_air! Converter from absolut vapour pressure to specific humidity (Lepparanta 2015)
                     ...
                     H_V = rho_air * (l_h + l_e) * Hv_CE * (qa - q0) * uv10 ! Through sublimation (solid to gas)
                  ...
                  heat = F_glob * exp(-lambda_snow*snwdp - lambda_snowice*wicedp - lambda_ice*bicedp) * beta_sol
                  rad0 = F_glob * exp(-lambda_snow*snwdp - lambda_snowice*wicedp - lambda_ice*bicedp) * (1 - beta_sol)

                  ! Heat flux into snow, ice or snowice layer.
                  ! Light absorption each layer
                  ! Leppäranta, M. (2014), Eq. 6.12
                  F_snow    = F_glob * (1 - exp(-lambda_snow*snwdp))
                  F_snowice = F_glob * (exp(-lambda_snow*snwdp) - exp(-lambda_snow*snwdp - lambda_snowice*wicedp))
                  F_ice     = F_glob * (exp(-lambda_snow*snwdp - lambda_snowice*wicedp) - exp(-lambda_snow*snwdp - lambda_snowice*wicedp - lambda_ice*bicedp))
                  if (F_snow < 0 .or. F_snowice < 0 .or. F_ice < 0 .or. heat < 0 .or. rad0 < 0) then
                     call error('Negative heat flux not allowed for melting.')
                  end if

                  !Light + other heat fluxes into top layer
                  if (snwdp > 0 .and. wicedp > 0 .and. bicedp > 0) then ! snow, snowice and ice
                     heat_snow = H_tot + F_snow
                     heat_snowice = F_snowice
                     heat_ice = F_ice
                  else if (snwdp > 0 .and. wicedp > 0 .and. bicedp == 0) then! snow and snowice
                     heat_snow = H_tot + F_snow
                     heat_snowice = F_snowice
                     heat_ice = 0
                  else if (snwdp > 0 .and. wicedp == 0 .and. bicedp > 0) then! snow and ice
                     heat_snow = H_tot + F_snow
                     heat_snowice = 0
                     heat_ice = F_ice
                  else if (snwdp == 0 .and. wicedp > 0 .and. bicedp > 0) then! snowice and ice
                     heat_snow = 0
                     heat_snowice = H_tot + F_snowice
                     heat_ice = F_ice
                  else if (snwdp == 0 .and. wicedp == 0 .and. bicedp > 0) then! ice
                     heat_snow = 0
                     heat_snowice = 0
                     heat_ice = H_tot + F_ice
                  else if (snwdp == 0 .and. wicedp > 0 .and. bicedp == 0) then! snowice
                     heat_snow = 0
                     heat_snowice = H_tot + F_snowice
                     heat_ice = 0
                  end if
               ...
               heat = heat + F_glob*beta_sol !MS: added term with beta_sol
               rad0 = F_glob * (1 - beta_sol) !FB, 2021: added term with beta_sol
               heat_snow = 0 ! Heat snow
               heat_snowice = 0 ! Heat snowice
               heat_ice = 0 ! Heat ice
         end if
         if (IceModel == 0) then
               if (lktmptop < 0 .and. heat < 0) then
                  heat = 0
                  lktmptop = 0
               end if
         end if
      ...
      if (WindDragModel == 1) then ! Constant wind drag coefficient
         stateC10 = c10
      else if (WindDragModel == 2) then ! Ocean model
         stateC10 = c10*(-0.000000712_RK*uv10**2 + 0.00007387_RK*uv10 + 0.0006605_RK)
      else if (WindDragModel == 3) then ! Lake model (Wüest and Lorke 2003)
         if (uv10 <= 0.1) then
               stateC10 = c10*0.06215_RK
         else if (uv10 <= 3.85_RK) then
               stateC10 = c10*0.0044_RK*uv10**(-1.15_RK)
         else ! Polynomial approximation of Charnock's law
               stateC10 = c10*(-0.000000712_RK*uv10**2 + 0.00007387_RK*uv10 + 0.0006605_RK)
         end if
      end if

      tau = stateC10*rho_air/rho_0*uv10**2
      u_taus = sqrt(tau)

      tx = stateC10*rho_air/rho_0*uv10*u10
      ty = stateC10*rho_air/rho_0*uv10*v10
   ...
   end subroutine Simstrat_SurfFlux



   subroutine Simstrat_FluxAdd( &
                              ! "in" arguments
                              ! ---------------------------
                              btpri    , usurf       , vsurf   , tmref   ,&
                              snwdp    , wicedp      , bicedp  , lwdns   ,&
                              csnow    , lsnow       , crain   , lrain   ,&
                              fsena    , lfevpa      , olrg    , sabg    ,&
                              ! "out" arguments
                              ! ---------------------------
                              heat_snow, heat_snowice, heat_ice, stateC10,&
   ...
   !     Omarjan Obulkasim (WMEJ), 03/2024: Separated this subroutine in order for it to be called by other lake models
   !------------------------------------------------------------------
   use MOD_Lake_Const, only : lambda_snow, lambda_snowice, lambda_ice, c10, rho_air, rho_0
   !==============================================================================
   ! ----- input variables -----
   real(RK), intent(in)     :: &
      btpri                   ,&! Beta primary
      ...
      vsurf                   ,&! Wind speed [m/s]
      snwdp                   ,&! Snow depth
      wicedp                  ,&! Ice thickness
      bicedp                  ,&! Ice thickness
      tmref                   ,&! air temperature
      csnow                   ,&! convective snowfall [kg/(m2 s)]
      lsnow                   ,&! large scale snowfall [kg/(m2 s)]
      crain                   ,&! convective rainfall [kg/(m2 s)]
      lrain                   ,&! large scale rainfall [kg/(m2 s)]               
      ...
      lfevpa                  ,&! Latent heat flux
      olrg                    ,&! Longwave radiation
      sabg                    ,&! Solar radiation absorbed by ground
      lwdns                     ! downward longwave radiation at surface [w/m2]

   ! ----- output variables -----
   real(RK), intent(out)    :: &
      rad0                    ,&! Solar radiation absorbed by the first water cell
      heat_snow               ,&! Heat snow
      heat_snowice            ,&! Heat snowice
      heat_ice                ,&! Heat ice
      stateC10                ,&! Drag coefficient
      u_taus                  ,&! Wind speed at the surface
      taux                    ,&! wind stress: E-W [kg/m/s**2]
      tauy                      ! wind stress: N-S [kg/m/s**2]

   ! ----- local variables -----
   real(RK)                 :: &
      F_glob                  ,&! Solar radiation absorbed by ground
      F_snow                  ,&! Light absorption in snow
      F_snowice               ,&! Light absorption in snowice
      F_ice                   ,&! Light absorption in ice
      H_A                     ,&! Heat flux from air to water
      H_W                     ,&! Heat flux from water to air
      H_K                     ,&! Heat flux from convection
      H_V                     ,&! Heat flux from evaporation
      H_tot                   ,&! Total heat flux
      heat                    ,&! Total heat flux
      u10                     ,&! Wind speed in x-direction
      v10                     ,&! Wind speed in y-direction
      uv10                    ,&! Wind speed at the surface
      T_atm                   ,&! Air temperature
   ...
   !==============================================================================
      ...
      ! Removal of solar short-wave radiation absorbed in first water cell
      rad0 = F_glob * (1 - btpri) !MS: added beta_sol
      ...
      ! -WMEJ there is a problem with the heat fluxes, they are not correct
      ! if (F_snow < 0 .or. F_snowice < 0 .or. F_ice < 0 .or. heat < 0 .or. rad0 < 0) then
      !     call error('Negative heat flux not allowed for melting.')
      ! end if
      ...
      u10 = usurf
      v10 = vsurf
      uv10 = sqrt(u10**2 + v10**2)
      T_atm = tmref
      precip = max(crain + csnow + lrain + lsnow, 0.0_RK)
      ...
      !-WMEJ Is the 'taux' in CoLML_SurfFlux the same as 'taux' here? There is considerable controversy here.
      tau = C10*rho_air/rho_0*uv10**2
      u_taus = sqrt(tau)
      taux = C10*rho_air/rho_0*uv10*u10
      tauy = C10*rho_air/rho_0*uv10*v10

    end subroutine Simstrat_FluxAdd
    ...
    ! AG 2014: revision
    !######################################################################
   SUBROUTINE SurfFlux(self, state,&
               ! "in" arguments
               ! ---------------------------
               ...
               stke1     , dzlaketop  , zlakebot  , lktmptop  ,&
...
!  ------------------------- input variables ---------------------------
   ...
   real(RK), intent(in)     :: &
      ...
      dzlaketop                ,&! thickness of top lake layer [m]
...
!================================================================================
      ...
      IF (.NOT.  DEF_External_Lake%DEF_USE_COLML_FLUX_SCHEME) THEN
      ...
      ELSE
         ...
         CALL CoLML_SurfFlux ( &
               ...
               hpbl       , dzlaketop   , zlakebot   , dzssbtop   ,&
...
!  ------------------------- inout variables ---------------------------
   real(r8), intent(inout)  :: &
      ...
      dzlake(nlake)            ,&! center depth of each layer [m]
...
!--------------------------------------------------------------------------
   ...
   SUBROUTINE Lake_Simstrat( &
               ...
               stateC10  , u_taus    , fgrnd1    , urban_call)    
...
!  ------------------------- inout variables ---------------------------
   ...
   real(RK), intent(inout)  :: &
      ...
      zissb(-nsnow:nsoil)     ,&! interface level below a "z_soisno" level (m)
...
!  ------------------------- output variables ---------------------------
   real(RK), intent(inout)  :: &
      ...
      fgrnd                 ,&! ground heat flux [W/m2]
      fgrnd1                ,&! ground heat flux abosrbed by first layer [W/m2]
      trad                  ,&! radiative temperature [K]
...
!  ------------------------- local variables ---------------------------
   real(RK)               :: &
      rad0                  ,&! net radiation at the surface [W/m2]
      filtw                 ,&! filtered wind speed [m/s]
      dzlaketop             ,&! thickness of top lake layer [m]
      zlakebot              ,&! depth of top lake layer [m]
      lktmptop              ,&! temperature of top lake layer [K]
      icefrtop              ,&! lake mass fraction of top lake layer that is frozen
      ...
      precip                ,&! precipitation rate [kg/(m2 s)]
...
!================================================================================
      ...
      lktmp = lktmp - tfrz
      tmice = tmice - tfrz
      precip = lrain + lsnow + crain + csnow
      pg_snow = csnow + lsnow

      ! Initialize all modules
      CALL reverse_variables ( &
            nlake   , dzlake   , zlake   , zilak   , lktmp   , uwatv   , vwatv   ,&
      ...
      IF (DEF_External_Lake%DEF_USE_SURFACE_LAYER) THEN
         rad0 = sabg * (1.0 - btpri)
         filtw = sqrt(usurf**2 + vsurf**2)    ! filter wind speed, may be used in the future
         call mod_forcing%FluxUpdate(simdata%model,&
               ! "in" arguments
               ! ---------------------------
               usurf     , vsurf       , filtw   , tmref     ,&
               tskin     , precip      , rad0    , fgrnd1    ,&
               stateC10  , u_taus      , taux    , tauy      ,&
               olrg      , fsena       , lfevpa  , lwdns     ,&
               hfice     , hfsnow      , hfsnowice)
      ELSE 
      ...
      CALL reverse_variables (    nlake   , dzlake   , zlake   , zilak   , lktmp   , uwatv   , vwatv   ,&
                                 lksal   , icefr   , tke     , eps     , num     , nuh     )
      ...
      ! after melt. remaining surface evaporation used for infiltration
         ...
         IF(lb < 0)THEN
            ...
            qsubl = fevpg - qseva
         ELSE
            qseva = min((1.-icefr(1))*1000.*dzlake(1)/dtlak, fevpg)
            qsubl = fevpg - qseva
         ENDIF
      ELSE
