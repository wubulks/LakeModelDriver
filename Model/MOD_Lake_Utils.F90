...
!****** Initialize FLake special variables
      tmmnw = 0.0
      DO i = 1, nlake
         tmmnw = tmmnw + lktmp(i)*(dzlake(i)/dplak)    !+WMEJ Consider the proportion of thickness of each layer
      ENDDO
      tmwml = lktmp(1)
      tmbot = lktmp(nlake) 
      tmups = lktmp(nlake) 
...
!  ------------------------- input variables ---------------------------
   ...
   real(r8), intent(inout) :: dzlake(nlake)     ! lake layer thickness [m]
...
!==============================================================================
      ...
      LayerScheme = DEF_External_Lake%DEF_LAKE_LAYER_SCHEME
      SELECT CASE (DEF_External_Lake%DEF_LAKE_LAYER_SCHEME)
         CASE(1)
      ...
      SELECT CASE (LayerScheme)
         ...
         !------------------------------
            ...
            dzlake(1) = DZ_COLML(1,scwat)                       !- The top layer thickness is set to 0.1 m
            sumrats = dplak / sum(DZ_COLML(1:nlake, scwat))
            dzlake(2:nlake-1) = DZ_COLML(2:nlake-1, scwat)*sumrats
            dzlake(nlake) = DZ_COLML(nlake, scwat)*sumrats - (dzlake(1) - DZ_COLML(1, scwat)*sumrats)

            !--- Calculate the lake layer interface depth
            zilak(1) = 0.0_r8
            DO i = 2, nlake
               zilak(i) = zilak(i-1) + dzlake(i-1)
            ENDDO
            zilak(nlake+1) = dplak

            !--- Calculate the lake layer node depth
            DO i = 1, nlake
               zlake(i) = zilak(i) + 0.5_r8 * dzlake(i)
            ENDDO
         ...
         !------------------------------
            ...
            DO i = 1, nlake
               dzlake = dplak * DZ_CSSPL(i,scwat) / sumrats
            ENDDO
...
!  ------------------------- input variables ---------------------------
   ...
   real(r8), intent(in)     :: dzlake(nlake), zilak(nlake+1)
...
!================================================================================
      ...
      IF (icedp < 0.00001) THEN 
         ...
         icefr(i) = 0.
         wice_lake(i) = 0.
         wliq_lake(i) = denh2o*dzlake(i)
         ENDDO        
      ELSE
         DO i = 1, nlake
            IF (icedp > zilak(i+1)) THEN
               icefr(i) = 1.
               wice_lake(i) = denice*dzlake(i)
               wliq_lake(i) = denh2o*dzlake(i)
            ELSE 
      ...
      ELSE
         DO i = 1, nlake
            IF (icedp > zilak(i+1)) THEN
               icefr(i) = 1.
            ELSE 
               icefr(i) = (icedp - zilak(i))/dzlake(i)
            ENDIF
         ENDDO
      ENDIF
#endif
